/*
 * @file yapp_genspplot.c
 * Program to generate a single-pulse diagnostic plot from given dedispersed
 *  time series data.
 *
 * @verbatim
 * Usage: yapp_genspplot [options] <data-file-dm0> ... <data-file-dmN-1>
...
int main(int argc, char *argv[])
{
    ...
    /* sort the input files and DM array in ascending DM order */
    for (i = 0; i < iNumDMs; ++i)
    {
        for (j = i; j < iNumDMs; ++j)
        {
    ...
    /* allocate for twice as many events */
    iMaxEvents = 2 * iNumRandEvents;
    if (iMaxEvents < stYUM.iTimeSamps)
    {
        iMaxEvents = stYUM.iTimeSamps;
    }
    g_pastPulses = (Pulse_t *) YAPP_Malloc((size_t) iMaxEvents,
                                         sizeof(Pulse_t),
                                         YAPP_FALSE);
    if (NULL == g_pastPulses)
    {
        (void) fprintf(stderr,
                       "ERROR: Memory allocation failed! %s!\n",
                       strerror(errno));
    ...
    while (iNumReads > 0)
    {
        ...
        for (i = 0; i < iNumSamps; ++i)
        {
            for (j = 0; j < iNumDMs; ++j)
            {
                ...
                //printf("%g, %g, %g\n", g_ppfBuf[j][i], fMedian, g_ppfBuf[j][i] / fRMS);
                if (fSNR > fThreshold)
                {
                    g_pastPulses[iNumEvents].dT = dTNow;
                    g_pastPulses[iNumEvents].fDM = pfDM[j];
                    g_pastPulses[iNumEvents].fSNR = fSNR;
                    ++iNumEvents;
                    if (iNumEvents == iMaxEvents)
                    {
                        /* allocate more memory */
                        (void) fprintf(stderr, "ERROR: Cannot store more events!\n");
                        x = 1;
                        break;
                        #if 0
                        (void) printf("INFO: Allocating more memory.\n");
                        iMaxEvents *= 2;
                        g_pastPulses = (Pulse_t *) YAPP_Malloc((size_t) iMaxEvents,
                                                             sizeof(Pulse_t),
                                                             YAPP_FALSE);
    ...
    (void) printf("Parsing data done. %d events of an expected %d detected. "
    ...
    {
        cpgslw(g_pastPulses[i].fSNR);
        cpgpt1((float) g_pastPulses[i].dT, g_pastPulses[i].fDM, -1);
    }
    cpgsci(PG_CI_DEF);
